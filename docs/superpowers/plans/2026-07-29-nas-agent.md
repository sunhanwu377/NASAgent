...
def test_read_tool_is_allowed_without_confirmation() -> None:
    tool = ToolDefinition(
        name="status", description="status", risk_level=RiskLevel.READ, handler=noop
    )
...
def test_destructive_tool_is_blocked_by_default() -> None:
    tool = ToolDefinition(
        name="delete_file", description="delete", risk_level=RiskLevel.DESTRUCTIVE, handler=noop
    )
    decision = SafetyPolicy(SafetySettings()).evaluate(tool)
    ...
    def evaluate(self, tool: ToolDefinition) -> SafetyDecision:
        if tool.risk_level == RiskLevel.READ:
            return SafetyDecision(
                allowed=True, requires_confirmation=False, reason="read operation"
            )
...
async def search_files(
    context: ToolContext, query: str, path: str | None = None
) -> dict[str, object]:
...
async def download_file(
    context: ToolContext, remote_path: str, local_path: str
) -> dict[str, object]:
...
async def test_step_runner_executes_expected_read_tool() -> None:
    ...
    step = PlanStep(
        id="s1", description="Get storage", risk="read", expected_tools=["get_storage_status"]
    )
...
async def test_agent_runs_plan_against_simulator() -> None:
    state = await run_agent_once(
        "check storage", adapter=SimulatorNasAdapter(), provider=FakeProvider()
    )
