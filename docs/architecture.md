...
Current configuration data flow: `nasagent config init` writes non-sensitive LLM/safety/observability settings to `~/.config/nasagent/config.toml`. If the user enters an LLM API key, the CLI persists it through `CredentialStore` as `llm.api_key` in `~/.local/share/nasagent/secrets.toml`, which is created and read with `0600` file permissions. `load_settings()` reads config TOML and `NASAGENT_` environment values into `NasAgentSettings`; it does not load secrets from `CredentialStore`. Those settings include core LLM/safety/observability models plus `AppEndpointSettings` records and `PluginSettings` records. App endpoint settings can point at a `credential_key`, with secret values for those keys kept in the credential store rather than in config TOML.
...
Current command data flow: `nasagent.plugins.commands.register_builtin_commands()` registers `/help`, `/apps`, `/plugins`, `/tools`, and `/containers`. Observed CLI command contexts for apps, plugins, containers, and chat slash commands create a platform context from loaded settings, load the built-in plugin, dispatch the matching slash command, and print the `CommandResult.message`. The chat REPL uses the same path when input starts with `/`, before local greeting, conversational LLM, or simulator execution handling.
...
2. `load_settings()` loads config TOML and `NASAGENT_` environment values; secrets remain in `CredentialStore` until code explicitly reads them.
3. Built-in plugins register capabilities in observed CLI command contexts; entry-point loading exists but is not necessarily invoked by every CLI or chat surface yet.
4. CLI commands and chat slash commands resolve through platform registries where a platform context has loaded those commands. Planner-selected tools currently use the agent graph's active `ToolRegistry`.
